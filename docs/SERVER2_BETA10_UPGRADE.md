# Upgrade Server2 to v0.1.0-beta.10
...
the superseded beta.5 or beta.6 tags. The qualified beta.7, beta.8 and beta.9 tags remain valid prior releases.
...
under `/var/lib/shopaware`. Use the full commit SHA from the successful beta.10
...
(
  ...
  test "$(git rev-parse 'v0.1.0-beta.10^{commit}')" = "$QUALIFIED_SHA"
  git switch --detach v0.1.0-beta.10
  test "$(cat VERSION)" = "0.1.0-beta.10"
...
The runtime check must report version `0.1.0-beta.10`, schema `6`, H.264 and passed
...
Confirm each camera retains its saved mode settings after restarting the backend
and after disabling/re-enabling the camera or changing its enabled modes. Saved
tuning must apply from the first processed frame. Regular users must remain
unable to edit mode settings. The container now runs `backend:app` directly;
`beta5_backend:app` remains a compatible entrypoint for existing launch scripts.
