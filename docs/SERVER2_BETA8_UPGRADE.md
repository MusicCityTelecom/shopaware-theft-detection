# Upgrade Server2 to v0.1.0-beta.8

Beta.8 preserves per-camera mode settings and schema 6. Upgrades from beta.4
perform the previously qualified additive schema-5-to-6 migration. Do not use
the superseded beta.5 or beta.6 tags. The qualified beta.7 tag remains valid.
...
under `/var/lib/shopaware`. Use the full commit SHA from the successful beta.8
...
(
  ...
  test "$(git rev-parse 'v0.1.0-beta.8^{commit}')" = "$QUALIFIED_SHA"
  git switch --detach v0.1.0-beta.8
  test "$(cat VERSION)" = "0.1.0-beta.8"
...
The runtime check must report version `0.1.0-beta.8`, schema `6`, H.264 and passed
...
In Mode Settings, switch quickly between two cameras with different saved
tuning. Confirm the fields belong to the selected camera. Change a value, use
Discard changes, and confirm the saved value returns. Save a test-camera change,
then confirm it persists after container recreation and RTSP reconnect. Regular
users must remain unable to edit mode settings.
